...
class DatabaseManager:
    ...
    def save_oi_data(self, symbol: str, timestamp: datetime, open_interest: float,
                    price: float, value_usdt: Optional[float] = None) -> bool:
        """
        保存持仓量数据
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute(
                    ...
                    (symbol, timestamp, open_interest, price, value_usdt)
                    VALUES (?, ?, ?, ?, ?)""",
    ...
    def get_recent_oi_data(self, symbol: str, minutes: int = 15) -> List[Dict[str, Any]]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                # 使用UTC+8时间确保一致性
                cutoff_time = (get_utc8_time() - timedelta(minutes=minutes)).isoformat()
                ...
                if recent_count == 0:
                    # 扩大搜索范围到30分钟
                    extended_minutes = 30
                    extended_cutoff = (datetime.utcnow() - timedelta(minutes=extended_minutes)).isoformat()
                    cursor.execute(
